...
const Navbar = () => {
  ...
  return (
    <div className="flex justify-between items-center w-full h-20 px-4 text-white bg-black fixed top-0 z-50">
      ...
      {/* Mobile Nav */}
      {nav && (
        <ul className="flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-black md:hidden transition-all duration-300 ease-in-out z-40">
          {links.map(({ id, name, to }) => (
            <li
              key={id}
